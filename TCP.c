...
//                    printf("Adding to list of sockets as %d\n" , i);
        ...
        for (int i = 0; i < max_clients; i++)
        {
            ...
            if (FD_ISSET( sd , &readfds))
            {
				...
				if(bytes_read>0) {
					if(serial_activate) {
                        int wrote_bytes =  write(serial_port,&bufer,bytes_read);
                    }
                    time_human();
                    char preview[128] = {"%s - TCP: %d - %02X %02X %02X %02X \n"};
                    sprintf(print_buffer,preview, time_string, bytes_read, bufer[0], bufer[1], bufer[2], bufer[3]);
